...
def histogram2image(hist, histSize, histImageWidth, histImageHeight, color):
    ...
    for i in range(histSize):
        cv2.rectangle(histImage, (int(i * binWidth), int(0)), (int((i + 1) * binWidth), int(hist[i])), color, -1)
...
hist2 = compute_histogram(img2, histSize, histRange)
...
histImage2 = histogram2image(
    hist2, histSize, histImageWidth, histImageHeight, color)
cv2.imshow("Histogram", histImage)
cv2.imshow("Histogram2", histImage2)
