...
namespace sofs19
{
     ...
     *
     *  An inode reference is retrieved from the list of free inodes  and is initialized.
     *
     ...
     *  \li when calling a function of any layer, use the version with prefix \c so.
     *
     ...
     *  \remarks
     *
     *  \li nothing should be done if the cache is not empty;
     *  \li the tail cache should only be used if there are no reference data blocks;
     *  \li only a single block should be processed, 
     *          even if it is not enough to fulfill the head cache;
     *  \li the block to be processed is the one pointed to by the \c head_blk field 
     *      of the superblock;
     *  \li after transferring a reference from A to B, the value in A should become NullReference;
     *  \li if after the replinish the head reference data block gets empty, 
     *          the head passes to the next, if one exists, and the previous is freed;
     *  \li in the previous situation, all entries of the freed data block should be NullReference; 
     *  \li when calling a function of any layer, use the version with prefix \c so.
     */
     ...
     *  \remarks
     *
     *  \li nothing should be done if the cache is not full;
     *  \li only a single block should be processed, 
     *          even if it is not enough to empty the tail cache;
     *  \li the block to be processed is the one pointed to by the \c tail_blk field 
     *      of the superblock;
     *  \li when calling a function of any layer, use the version with prefix \c so.
     */
