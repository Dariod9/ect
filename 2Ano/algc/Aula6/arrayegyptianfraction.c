...
PtEgyptianFraction EgyptianFractionCreate (PtFraction pfraction)	/* construtor inicializador - initializer constructor */
{
	...
	if(FractionGetNumerator(pfraction)<=0){
		Error =NOT_PROPER;
	 	return NULL;
	 }
	...
	while(i<MAX_SIZE)
	{
		...
		frac->Array[i++] = ptFrac;
		if(copy == NULL) break;
		if(FractionIsNull(copy)) {comp=1;break;}	
...
PtEgyptianFraction EgyptianFractionCopy (PtEgyptianFraction pegyp)	/* construtor c�pia - copy constructor */
{
	...
	PtFraction frac2 = EgyptianFractionToFraction(pegyp);

	PtEgyptianFraction Frac = EgyptianFractionCreate(frac2);
	if(Frac == NULL){
		Error = NO_MEM;
		return NULL;
		}
