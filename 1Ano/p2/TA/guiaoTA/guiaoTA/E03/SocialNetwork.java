...
public class SocialNetwork {
  ...
  private int numRequests = 0;
	...
	public void expand(){
		...
		FriendshipRequest[] requests2= new FriendshipRequest[requests.length+sizeExpansion];
		assert requests2.length>requests.length;
		System.arraycopy(requests, 0, requests2, 0, numRequests);
  ...
  public void addFriendshipRequest(String m1, String m2) {
    //...
    assert personIsMember(m1) && personIsMember(m2) :("Membros não existem!");
  ...
  public int numPendingRequests(String memberName) {
    //...
		...
		int cont=0;
		int x= memberIndex(memberName);
		
		for(int i=0; i<requests.length;i++){
		 if(requests[i]!=null){
			if(requests[i].pending()){
			
				if(requests[i].requester() == x){
					cont++;
					}
				if(requests[i].requested() == x){
					cont++;
					}
  ...
  public String oldestFriend(String memberName) {
    ...
    for(int i=0; i<=requests.length;i++){
		if(requests[i].accepted()){
			...
			if(requests[i].requested() == x){
				return members[requests[i].requester()].name();
				}
			
			else if(requests[i].requester() == x){
				return members[requests[i].requested()].name();
				}
