...
public class Data {
  ...
  public Data(String s){
	...
	//~ int d=Integer.parseInt(a[2]);
		...
		//~ if(!dataValida(d,m,a1)) System.exit(2);

		 this.ano=Integer.parseInt(a[0]);
		 this.mes=Integer.parseInt(a[1]);
		 this.dia=Integer.parseInt(a[2]);
  ...
  public int compareTo(Data s){
	  ...
	  if(this.ano>s.ano()) x=1;
	  
	  if(this.ano==s.ano()){
		  if(this.mes>s.mes()) x=1;
		  else if(this.mes<s.mes()) x=-1;
		  else if(this.mes==s.mes()){
			  if(this.dia>s.dia()) x=1;
			  else if(this.dia<s.dia()) x=-1;
			  else if(this.dia==s.dia()) x=0;		  
			  }
		  }
